...
async def admin_stats(db: AsyncSession = Depends(get_db)) -> AdminDashboardStats:
    today = date.today()
    ...
    scans_all_time = await _scan_stats(
        db, customer_count=total_customers, since=None
    )
    scans_month = await _scan_stats(
        db, customer_count=customers_this_month, since=month_start
    )
    ...
    top_vendors = [
        TopVendor(vendor_id=r.id, vendor_name=r.name, customer_count=r.count) for r in top_rows
    ]
